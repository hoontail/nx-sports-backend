...
exports.getSportsListForUser = async (req, res) => {
  ...
  if (gameType !== "inplay") {
    ...
    try {
      ...
      const matchListWithOdds = await Promise.all(
        findSportsMatches.map(async (match) => {
          const key = `odds:${match.match_id}`;
          let oddsObj = {};

          const redisHash = await redisClient.hGetAll(key);

          oddsObj = Object.values(redisHash)
            .map((val) => {
              try {
                const odds = JSON.parse(val);

                if (!odds.is_market_stop && !odds.is_odds_stop) {
                  const sportsMarket = findSportsMarket.find(
                    (x) => x.market_id === odds.market_id
                  );

                  return {
                    ...odds,
                    sports_market: sportsMarket || null,
                  };
                }
              } catch {
                return null;
      ...
      const sportsCount = getSportsCount(matchListWithOdds, true);

      return res.status(200).send({
        list: matchListWithOdds,
        count: sportsCount,
      });
    } catch (err) {
      console.log(err);
